  ...
  for(rp in c(-10:-1, r_start:10)){
    ...
    for(p1 in c("AFR", "AMR", "EAS", "EUR", "SAS")){
      for(p2 in c("AMR", "EAS", "EUR", "SAS")){
        ...
        kl1 <- sum(df[[p1]][,kl12_name])
        kl2 <- sum(df[[p2]][, kl21_name])
        chi1 <- sum(df[[p1]][,chi12_name])
        chi2 <- sum(df[[p2]][,chi21_name])
    ...
    #control stats
    for(p1 in c("AFR", "AMR", "EAS", "EUR", "SAS")){
      chi_stat <- sum(df[[p1]]$chi_c)
      kl_stat <- sum(df[[p1]]$kl_c)
      final <- bind_rows(final, 
                           data.frame(rp = rp,
                                      pop1 = p1,
                                      pop2 = "control",
...
df %>%
  ...
  geom_point() +
  geom_line() +
  ggtitle("Single Position KL", paste0("AMR: ", subtype)) +
  labs(color='Source Population') +
  xlab("Relative Position") +
  ylab("KL Divergence")
...
df %>%
  ...
  geom_point() +
  geom_line() +
  ggtitle("Single Position KL", paste0("EAS: ", subtype)) +
  labs(color='Source Population') +
  xlab("Relative Position") +
  ylab("KL Divergence")
...
df %>%
  ...
  geom_point() +
  geom_line() +
  ggtitle("Single Position KL", paste0("EUR: ", subtype)) +
  labs(color='Source Population') +
  xlab("Relative Position") +
  ylab("KL Divergence")
...
df %>%
  ...
  geom_point() +
  geom_line() +
  ggtitle("Single Position KL", paste0("SAS: ", subtype)) +
  labs(color='Source Population') +
  xlab("Relative Position") +
  ylab("KL Divergence")
```
