...
chi_sq_ind <- function(population, subtype, p1, p2){
  df_tab <- singleton_count_2pos(population, subtype, p1, p2)
  stat_val <- (chisq.test(df_tab)$statistic %>% unname())
  return(stat_val)
}

chi_sq_ind_plot <- function(population, subtype){
  p <- heatmap_plot(population, subtype, chi_sq_ind, "Chi Sq Test of Independence: ", "Chi Sq Statistic")
  return(p)
}
```
...
chi_sq_gof_plot <- function(population, subtype){
  p <- heatmap_plot(population, subtype, chi_sq_gof_pair, 
                    "Control Di-Nucleotide Rate: ", "Chi Sq Statistic")
  return(p)
}

chi_sq_re_plot <- function(population, subtype){
  p <- heatmap_plot(population, subtype, chi_sq_re_pair, 
                    "Control Di-Nucleotide Rate: ", "Relative Entropy")
  return(p)
}
```
...
control_mult_int_plot <- function(population, subtype, r_start = 1, type = "gof"){
  if(type == "gof") {
...
plot_control_mult_res <- function(population, subtype, p1, p2){
  df <- control_int_mult_res(population, subtype, p1, p2)
  p <- df %>%
    ggplot(aes(x = p2, y = p1, fill = statistic)) +
    geom_tile() +
