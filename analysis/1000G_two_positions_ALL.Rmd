...
chi_sq_re_pair <- function(population, subtype, p1, p2){
  data_dir <- "/net/snowwhite/home/beckandy/research/1000G_LSCI/output/all_count_2_pos/"
  f_name <- paste0(data_dir, population, "/", subtype, "_p", p1, "_q", p2, ".csv")
  df <- read_csv(f_name, col_types = cols()) %>%
    filter(singletons > 0) %>%
...
```{r}
chi_sq_gof_all <- function(population, subtype, stat_func = chi_sq_gof_pair){
  final <- data.frame(p1 = numeric(),
                      p2 = numeric(),
                      statistic = numeric())
  for(i in c(-10:-1,1:9)){
...
```{r}
chi_sq_gof_plot <- function(population, subtype, stat_func = chi_sq_gof_pair, stat_lab = "Chi Square Statistic"){
  df <- chi_sq_gof_all(population, subtype, stat_func)
  subtype2 <- str_replace(subtype, "_", " -> ")
  p <- df %>%
    ggplot(aes(x = p2, y = p1, fill = statistic)) +
    geom_tile() +
...
```{r}
subtype <- "AT_CG"
p <- chi_sq_gof_plot("ALL", subtype, chi_sq_re_pair, "RE")
print(p)
```
...
```{r}
subtype <- "AT_TA"
p <- chi_sq_gof_plot("ALL", subtype, chi_sq_re_pair)
print(p)
```
...
Let's try it out with the multiplicative version:
...
control_int_mult_pair <- function(population, subtype, p1, p2, type = "gof") {
  data_dir <- "/net/snowwhite/home/beckandy/research/1000G_LSCI/output/all_count_2_pos/"
  f_name <- paste0(data_dir, population, "/", subtype, "_p", p1, "_q", p2, ".csv")
  df <- read_csv(f_name, col_types = cols()) %>%
    filter(singletons > 0)
...
control_int_mult_all <- function(population, subtype, type = "gof"){
  final <- data.frame(p1 = numeric(),
                      p2 = numeric(),
                      statistic = numeric())
  for(i in c(-10:-1,1:9)){
    for(j in (i+1):10){
      if(j == 0) next
      stat_val <- control_int_mult_pair(population, subtype, i, j, type = type)
      final <- bind_rows(final, data.frame(p1 = i,
                                           p2 = j,
                                           statistic = stat_val))
    }
...
control_mult_int_plot <- function(population, subtype, type = "gof"){
  df <- control_int_mult_all(population, subtype, type = type)
  subtype2 <- str_replace(subtype, "_", " -> ")
  p <- df %>%
    ggplot(aes(x = p2, y = p1, fill = statistic)) +
    geom_tile() +
    ggtitle(paste0("Control Interaction GOF: ", subtype2),
            paste0("Population: ", population,
                   "; Min: ", round(min(df$statistic), 2), 
                   "; Max: ", round(max(df$statistic), 2)))+
    xlab("Relative Position 2") +
