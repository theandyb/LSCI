...
```{r}
chi_sq_gof_plot <- function(population, subtype, stat_func = chi_sq_gof_pair){
...
control_int_mult_pair <- function(population, subtype, p1, p2) {
  ...
  exp_s <- exp_s / sum(exp_s) # normalize
  exp_s <- exp_s * sum(singleton_tab)
  chi_sq <- (exp_s - singleton_tab)^2 / exp_s
  return(sum(chi_sq))
}

control_int_mult_all <- function(population, subtype){
  final <- data.frame(p1 = numeric(),
                      p2 = numeric(),
                      statistic = numeric())
  for(i in c(-10:-1,1:9)){
    for(j in (i+1):10){
      if(j == 0) next
      stat_val <- control_int_mult_pair(population, subtype, i, j)
      final <- bind_rows(final, data.frame(p1 = i,
                                           p2 = j,
                                           statistic = stat_val))
    }
...
control_mult_int_plot <- function(population, subtype){
  df <- control_int_mult_all(population, subtype)
  subtype2 <- str_replace(subtype, "_", " -> ")
  p <- df %>%
    ggplot(aes(x = p2, y = p1, fill = statistic)) +
    geom_tile() +
...
```{r}
p <- control_mult_int_plot("ALL", "AT_GC")
print(p)
```
