...
df_resid_control <- read_subtype_pop_range_resid("control_control", "AT_CG_AFR_EUR", size = 10) %>%
  type_convert()

df_resid <- df_resid %>%
  inner_join({df %>% select(offset, re)}, by = c("pos" = "offset"))

df_resid_control <- df_resid_control %>%
  inner_join({df_control %>% select(offset, re)}, by = c("pos" = "offset"))
...
df %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
  mutate(test_point = abs(offset) > 500) %>%
  group_by(test_point) %>%
  summarize(pct_sig = sum(is_sig) / n())


df %>%
  filter(abs(offset) < 501) %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
  ggplot(aes(x = offset, y = re, color = is_sig)) +
  geom_point() +
  scale_y_log10() +
  ...
  theme(text = element_text(family = "Times New Roman"),
        legend.position = c(.85, .8)) +
  geom_point(data = df_control, aes(x= offset, y = re), colour = "green")


```

New: let's look at the total variation distance at flanking positions between the singleton and control multinomials:

```{r}
df %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
  ggplot(aes(x = offset, y = tot_var, color = is_sig)) + 
  geom_point() +
  scale_y_log10()

df %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
  ggplot(aes(x = offset, y = max_var, color = is_sig)) + 
  geom_point() +
  scale_y_log10()

df %>%
  ggplot(aes(x = re, y = tot_var)) +
  geom_point()

df %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
  ggplot(aes(x = re, y = max_var, color = is_sig)) +
  geom_point()
...
df %>%
  mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>% 
  filter(!is_sig) %>%
  arrange(desc(max_var)) %>%
  head()

df %>%
  mutate(re_sq = sqrt(0.5 * re)) %>%
  ggplot(aes(x = re_sq, y = tot_var)) +
  geom_point()
```
...
df %>%
  mutate(re_sq = sqrt(1 - exp(-re))) %>%
  ggplot(aes(x = re_sq, y = 0.25*tot_var)) +
  geom_point()
```
...
Let's get some information at the +1, where we see the largest deviance:
...
df_resid %>%
  ...
  mutate(pct_s = singletons / sum(singletons),
         pct_c = controls / sum(controls)) %>%
  mutate(d_s = abs(pct_s - pct_c)) %>%
  pull(d_s) %>%
  mean()
```
...
df_resid %>% 
  ...
  mutate(p_sing = singletons / sum(singletons), 
         p_controls = controls / sum(controls)) %>%
  mutate(l_1 = abs(p_sing - p_controls)) %>%
  mutate(shift_pct = l_1) %>%
  pull(shift_pct) %>%
  mean()
```
...
df %>%
  ...
  theme(text = element_text(family = "Times New Roman"),
        legend.position = c(.85, .8)) +
  ggtitle("A → G", "Beyond 250 From Center") +
  geom_hline(yintercept = 17.47)
...
And now total variation and max variation:
...
For this sub type we observe the strongest signal at the +1 relative position. Below we observe that this signal is driven by an enrichment of T and depletion of A and G. Also note that we also have an enrichment of C at the +1, although this is far more mild than observed in the previous two subtypes.
...
And just to be absolutely sure that none of our singletons or controls have a G at the +1 flanking position:

```{r}
df_resid %>% filter(pos == 1) %>%
  knitr::kable()
```
...
```{r}
df <- read_subtype_pop_res("ALL", "GC_AT")
...
Here, for this subtype we observe the strongest signals at -1 and +2. Note again that we have conditioned on this not being at CpG sites. Below we see that the primary driver of the signal is a depletion of T and an enrichment of A at the -1.

```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "GC_AT", size = 10)  %>%
  type_convert()
...
```{r}
df <- read_subtype_pop_res("ALL", "GC_CG")
...
```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "GC_CG", size = 10)  %>%
  type_convert()
...
```{r}
df <- read_subtype_pop_res("ALL", "cpg_GC_TA")
...
```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "cpg_GC_TA", size = 10)  %>%
  type_convert()
...
df %>%
  ...
  guides(color = guide_legend(title = "Significant")) +
  theme_classic() +
  ggtitle("CpG → ApG", "Beyond 250 Bases from Center") +
  theme(text = element_text(family = "Times New Roman"),
        legend.position = c(.85, .8)) +
...
```{r}
df <- read_subtype_pop_res("ALL", "cpg_GC_AT")
...
```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "cpg_GC_AT", size = 10)  %>%
  type_convert()
...
```{r}
df <- read_subtype_pop_res("ALL", "cpg_GC_CG")
...
```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "cpg_GC_CG", size = 10)  %>%
  type_convert()
...
```{r}
df <- read_subtype_pop_res("ALL", "all_GC_TA")
...
```{r}
df_resid <- read_subtype_pop_range_resid("ALL", "all_GC_TA", size = 10)  %>%
  type_convert()
...
```{r}
df <- read_subtype_pop_res("ALL", "all_GC_AT")
...
df_resid <- read_subtype_pop_range_resid("ALL", "all_GC_AT", size = 10)  %>%
  type_convert()
...
## Comparing results to Control-Control Results
...
control_control_analysis <- function(subtype, dir_con_con){
  df_control <- load_con_con(subtype, dir_con_con)
  df <- read_subtype_pop_res("ALL", subtype)
  df_resid <- read_subtype_pop_range_resid("ALL", subtype, size = 10)  %>%
    type_convert()
  df_resid <- df_resid %>%
    inner_join({df %>% select(offset, re)}, by = c("pos" = "offset"))
  ...
  p3 <- df %>%
    mutate(is_sig = dev > qchisq(p=.05, df=3, lower.tail=FALSE)) %>%
    mutate(group = ifelse(is_sig, "Significant", "Non-significant")) %>%
    ggplot(aes(x = offset, y = re, color = group)) +
    geom_point() +
    geom_point(data = df_control, mapping =aes(x = offset, y = re), alpha = 0.3, color = "green") +
    scale_y_log10() +
    xlab("Relative Position") +
    ylab("Log10 Relative Entropy") +
    guides(color = guide_legend(title = "Significant")) +
  ...
  return(list("p1" = p1, "p2" = p2, "p3" = p3, "df" = df, "df_control" = df_control))
}

res <- control_control_analysis(subtype, dir_con_con)
print(res$p1)
print(res$p2)
print(res$p3 + geom_hline(yintercept = as.numeric(quantile(res$df_control$re, 0.95))))

c_p_sig <- as.numeric(quantile(res$df_control$re, 0.95))

res$df %>%
  filter(abs(offset) >= 215) %>%
  pull(re) %>%
  {sum(.>c_p_sig) / length(.)}
...
res$df %>%
  filter(abs(offset) >= 500) %>%
  pull(re) %>%
  {sum(.>c_p_sig) / length(.)}

res$df %>%
  filter(abs(offset) >= 750) %>%
  pull(re) %>%
  {sum(.>c_p_sig) / length(.)}

res$df %>%
  filter(abs(offset) >= 886) %>%
  pull(re) %>%
  {sum(.>c_p_sig) / length(.)}
...
res$df %>%
  filter(abs(offset) >= 975) %>%
  pull(re) %>%
  {sum(.>c_p_sig) / length(.)}

print(max(res$df_control$re))
```
