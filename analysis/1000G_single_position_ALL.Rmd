...
get_all_cohen <- function(sp){
  df1 <- statistic_by_position("AT_CG", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-CG")
  df2 <- statistic_by_position("AT_GC", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-GC")
  df3 <- statistic_by_position("AT_TA", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-TA")
  
  df4 <- statistic_by_position("GC_AT", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-AT")
  df5 <- statistic_by_position("GC_TA", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-TA")
  df6 <- statistic_by_position("GC_CG", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-CG")
  
  df7 <- statistic_by_position("cpg_GC_AT", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "cpgGC-AT")
  df8 <- statistic_by_position("cpg_GC_TA", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "cpgGC-TA")
  df9 <- statistic_by_position("cpg_GC_CG", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
...
```{r echo=FALSE, message=FALSE}
st <- "cpg_GC_AT"
p1 <- plot_pos_stat(st, "ALL", r_start = 2)
