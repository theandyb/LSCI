...
*Note: this is only present as material which we could choose to later include in the appendix*
...
cpg_stat_by_pos <- function(sp){
  df <- load_all_results("all_GC_AT", sp)
  df <- df %>%
    select(rp, deviance, chi_s) %>%
    group_by(rp) %>%
...
kl_by_position <- function(subtype, sp, r_start = 1){
...
gof_by_position <- function(subtype, sp, r_start = 1){
  ...
  for(i in c(-10:-1, r_start:10)){
    ...
    final <- bind_rows(final,
                       data.frame(pos = c(i,i), 
                                  ...
                                  deviance = c(NA, sum(df$deviance, na.rm = T)),
...
plot_gof_pos_stat <- function(subtype, sp, r_start = 1){
  df <- gof_by_position(subtype, sp, r_start)
  p <-df %>%
    ggplot(aes(x = pos, y = chi_sq, colour = type)) +
    geom_point() +
    geom_line() +
    ggtitle(paste0("1KGP Single-Position Results: ", str_replace_all(subtype, "_", "-")),
...
plot_control_gof_pos_stat <- function(subtype, sp, r_start = 1){
  df <- gof_by_position(subtype, sp, r_start)
  p <-df %>%
    filter(type == "Control") %>%
    ggplot(aes(x = pos, y = chi_sq)) +
    geom_point() +
    geom_line() +
    ggtitle(paste0("1KGP Single-Position Results: ", str_replace_all(subtype, "_", "-")),
            paste0("Population: ", sp)) +
...
plot_deviance_pos_stat <- function(subtype, sp, r_start = 1){
  df <- gof_by_position(subtype, sp, r_start)
  p <-df %>%
    filter(type == "Control") %>%
    ggplot(aes(x = pos, y = deviance)) +
    geom_point() +
    geom_line() +
    ggtitle(paste0("1KGP Single-Position Results: ", str_replace_all(subtype, "_", "-")),
            paste0("Population: ", sp)) +
...
plot_pos_cohen <- function(subtype, sp, r_start = 1){
  df <- gof_by_position(subtype, sp, r_start)
  p <-df %>%
...
plot_pos_kl <- function(subtype, sp, r_start = 1){
  df <- kl_by_position(subtype, sp, r_start)
  p<- df %>%
    ggplot(aes(x = pos, y = kl_stat)) +
    geom_point() +
    geom_line() +
    xlab("Relative Position") + 
    ylab("Relative Entropy") +
    ggtitle(paste0("Relative Entropy by Position: ", subtype))
  return(p)
}

get_all_cohen <- function(sp){
  df1 <- gof_by_position("AT_CG", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-CG")
  df2 <- gof_by_position("AT_GC", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-GC")
  df3 <- gof_by_position("AT_TA", sp) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "AT-TA")
  
  df4 <- gof_by_position("GC_AT", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-AT")
  df5 <- gof_by_position("GC_TA", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-TA")
  df6 <- gof_by_position("GC_CG", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "GC-CG")
  
  df7 <- gof_by_position("cpg_GC_AT", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "cpgGC-AT")
  df8 <- gof_by_position("cpg_GC_TA", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
    mutate(subtype = "cpgGC-TA")
  df9 <- gof_by_position("cpg_GC_CG", sp, r_start = 2) %>%
    filter(type == "Control") %>%
    select(pos, cohen) %>%
...
plot_all_cohen <- function(sp){
  ...
  p <-df %>%
    ...
    geom_point() +
    geom_line() +
    ggtitle(paste0("1KGP Single-Position Results"),
            sp) +
    xlab("Relative Position") +
    ylab("Cohen's w") +
...
st <- "AT_CG"
p1 <- plot_deviance_pos_stat(st, "ALL")
p2 <- plot_pos_cohen(st, "ALL")

tikz(paste0("/net/snowwhite/home/beckandy/research/documentation/LSCI/output/tikz/single_pos/1KGP_chisq_", st, ".tex"),
     width = width, height = height)
p1
dev.off()

tikz(paste0("/net/snowwhite/home/beckandy/research/documentation/LSCI/output/tikz/single_pos/1KGP_cohen_", st, ".tex"),
     width = width, height = height)
p2
dev.off()
...
gof_by_position("AT_CG", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```{r results='asis', echo=FALSE}
gof_by_position("AT_GC", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```{r results='asis', echo=FALSE}
gof_by_position("AT_TA", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```
```{r results='asis', echo=FALSE}
gof_by_position("GC_AT", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```{r echo=FALSE, message=FALSE}
st <- "cpg_GC_AT"
p1 <- plot_gof_pos_stat(st, "ALL", r_start = 2)
p2 <- plot_pos_cohen(st, "ALL",  r_start = 2)
...
```
```{r results='asis', echo=FALSE}
gof_by_position("cpg_GC_AT", "ALL", r_start = 2) %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```
```{r results='asis', echo=FALSE}
gof_by_position("GC_TA", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```
```{r results='asis', echo=FALSE}
gof_by_position("cpg_GC_TA", "ALL", r_start = 2) %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```
```{r results='asis', echo=FALSE}
gof_by_position("GC_CG", "ALL") %>% spread(type, chi_sq) %>% knitr::kable()
```
...
gof_by_position("cpg_GC_CG", "ALL", r_start = 2) %>% spread(type, chi_sq) %>% knitr::kable()
```
...
```{r echo=FALSE, message=FALSE}
p <- plot_all_cohen("ALL")

tikz(paste0("/net/snowwhite/home/beckandy/research/documentation/LSCI/output/tikz/single_pos/cohen_all_1kgp.tex"),
     width = width, height = height)
p
dev.off()
...
```{r echo=FALSE, message=FALSE}
st <- "AT_CG"
p1 <- plot_nuc_cont_by_position(st, "ALL", type="ct")
p2 <- plot_signed_nuc_by_pos(st, "ALL", type = "ct")
p1
p2

tikz(paste0("/net/snowwhite/home/beckandy/research/documentation/LSCI/output/tikz/single_pos/1KGP_nuc_sp_", st, ".tex"),
     width = width, height = height)
p1
dev.off()

tikz(paste0("/net/snowwhite/home/beckandy/research/documentation/LSCI/output/tikz/single_pos/1KGP_signed_nuc_sp_", st, ".tex"),
     width = width, height = height)
p2
dev.off()
```
...
```{r echo=FALSE, message=FALSE}
st <- "cpg_GC_AT"
p1 <- plot_nuc_cont_by_position(st, "ALL", type="ct", r_start = 2)
p2 <- plot_signed_nuc_by_pos(st, "ALL", type = "ct", r_start = 2)
p1
p2
